...
function StudioDetailPage() {
  ...
  const { data: studioRes, isLoading: isStudioLoading, isError: isStudioError } = useQuery({
    queryKey: ['studio-detail', id],
    queryFn: () => studioApi.getById(id!),
    enabled: !!id,
  });

  // Fetch media produced by this studio
  const { data: mediaRes, isLoading: isMediaLoading } = useQuery({
    queryKey: ['studio-media-list', id],
    queryFn: () => mediaApi.getAll({ studio: id, limit: 100 }),
    enabled: !!id,
  });
  ...
  // Calculate average score & highest rated title
  let scoreSum = 0;
  let scoredCount = 0;
  let highestRatedMedia: Media | null = null;

  mediaList.forEach((m) => {
    if (m.score != null) {
      scoreSum += Number(m.score);
      scoredCount++;
      if (!highestRatedMedia || Number(m.score) > Number(highestRatedMedia.score || 0)) {
        highestRatedMedia = m;
      }
    }
  });
  ...
  if (isStudioLoading || isMediaLoading) {
  ...
  if (isStudioError || !studio) {
    return (
      <EmptyState
        ...
        description="The studio you are looking for does not exist."
        emoji="⚠️"
        action={
          <button
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      >
        <ArrowLeft size={14} /> Back to Studios
      </button>
      ...
      <div
        ...
        style={{
          background: isDark
            ? 'linear-gradient(135deg, #1d1035, #101e35, #1d1035)'
            : 'linear-gradient(135deg, #fae8ff, #f3e8ff, #ecfeff)',
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium mt-1 max-w-xl" style={{ color: 'var(--text-secondary)' }}>
                {studio.description || 'No detailed overview available for this studio.'}
              </p>
              {studio.website_url && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex items-center gap-1 text-xs font-bold text-purple-500 hover:underline mt-2"
                >
      ...
      {mediaList.length === 0 ? (
        <EmptyState
          title={`No titles produced by "${studio.name}" yet`}
          description="Edit titles in your watchlist to assign this studio!"
          emoji="🎬"
        />
      ) : (
            ...
            >
              <div className="flex gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <h3
                    className="text-sm font-bold truncate group-hover:text-purple-500 transition-colors"
                    style={{ fontFamily: 'var(--font-heading)', color: 'var(--text-primary)' }}
                  >
                    {item.title}
                  </h3>
